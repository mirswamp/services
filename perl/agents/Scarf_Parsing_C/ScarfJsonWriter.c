...
int ScarfJSONWriterAddBug(ScarfJSONWriter * writerInfo, BugInstance * bug, entry *list)
{
    ...
    if ( bug->methods != NULL && getAttributeValid("methods")) {
        int methodId = 1;
    ...
    if ( bug->locations != NULL && getAttributeValid("locations")) {
	int locID = 1;
    ...
    if ( (inst.xPath != NULL || inst.lineNum.start != 0 || inst.lineNum.end != 0) && getAttributeValid("locations")) {
