...
if ( isSWAMPRunning() ) {
	...
	$log->info("Attempting to launch run $execrunid");
        if ( !callDoRun($execrunid) ) {
            # If the call failed, save the run in the queue
            saveRun($execrunid);
            updateRunStatus($execrunid, 'Unable to run, queued', 1);
	    $log->info("Unable to launch run $execrunid. Added run to the queue.");
        }
        else {
            updateRunStatus($execrunid, 'Enqueued');
	    $log->info("Run $execrunid successfully launched.");
    ...
    if ( defined($execrunid) ) {
        saveRun($execrunid);
        updateRunStatus($execrunid, 'Unable to run, queued', 1);
	$log->info("SWAMP is Off at the moment. Run $execrunid has been added to the queue.");
    }
    if ( defined($drain) ) {
...
sub drainSwamp {
    my $aref = loadQueue();
    $log->info("drainSwamp: queue has $#{$aref} items in it");
    foreach my $idx ( 0 .. $#{$aref} ) {
        if ( !defined( $aref->[$idx] ) ) {
            next;
        }

	$log->info("processing run " . $aref->[$idx] . " from the queue.");

        # If the call succeeded, delete the
        # item from the queue
        if ( callDoRun( $aref->[$idx] ) ) {
	    $log->info("Run " . $aref->[$idx] . " has been successfully launched from the queue.");
            $aref->[$idx] = 'done';
        }
	else {
	    $log->info("Run " . $aref->[$idx] . " failed to launch and remains in the queue.");
	}
