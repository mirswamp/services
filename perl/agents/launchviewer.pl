...
if (($viewer_name =~ /CodeDX/ixsm) || ($viewer_name =~ /ThreadFix/ixsm)) {
	print "SUCCESS\n";
    chdir(q{/});
...
elsif ( $viewer_name =~ /ThreadFix/isxm ) {
    $exitCode = doViewerVM();
}
else {
...
Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name Exit launchviewer: $exitCode");
exit $exitCode;

sub doViewerVM {
    my $start_time = time();
	Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name Start launchviewer");
    my $retCode     = NOTOK;
    my $launchTried = 0;
    $log->info("doViewerVM $retCode");
...
#}
	...
	$log->info("Calling isViewerAvailable: $project_name");
    	...
    	my $viewerStatusValue = $viewerStatus->{'ready'};
        if (defined($viewerStatus->{'error'})) {
		$viewerStatusValue = 'error';
	}
	if (! defined($oldViewerStatusValue) || ("$viewerStatusValue" ne "$oldViewerStatusValue")) {
		Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name isViewerAvailable returns: $viewerStatusValue");
		$oldViewerStatusValue = $viewerStatusValue;
	}
	$log->info("Back from isViewerAvailable: $project_name viewerStatus: ", $viewerStatus->{'ready'});
        ...
        if ( $viewerStatus->{'ready'} == 1 ) {
	    	...
	    	Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name Found available viewer");
            $viewerIsAvailable = 1;
            $retCode           = OK;
            last;
        }

        $log->info("doViewerVM isViewerAvailable[", $launchTried, "]: no viewer available $retCode");
	    ...
	    Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name Calling launchViewer");
            $retCode = launchViewer(%launchMap);
	    $log->info("LAUNCHVIEWER TIME: Back from launchViewer RPC: $retCode seconds: ", time() - $launch_time);
        }
        if ( $retCode != OK ) {
	    Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name Unable to launch viewer $retCode");
    ...
    $log->info("LAUNCHVIEWER TIME: doViewerVM:back from isViewerAvailable attempts: $launchTried - retCode: $retCode seconds: ", $available_time, " sleep: $sleep_total process: ", $available_time - $sleep_total);
    ...
    if ( $retCode == OK ) {
	    ...
	    if ($viewer_name eq 'CodeDX') {
	    	...
	    	$log->info("LAUNCHVIEWER TIME: $viewer_name Back from uploadanalysisrun systemcall: $retCode seconds: ", time() - $upload_time);
	    }
	    elsif ($viewer_name eq 'ThreadFix') {
	    	$log->info("Calling $viewer_name uploadanalysisrun systemcall");
	    	my $upload_time = time();
            	$retCode = threadfix_uploadanalysisrun(
                	$viewerStatus->{'address'},
                	$viewerStatus->{'apikey'},
                	$viewerStatus->{'urluuid'},
                	$package_name, \@file_path
        ...
        if ( $retCode == OK ) {
            updateviewerinstance({'vieweruuid' => $viewer_uuid,
                ...
                'vieweraddress' => $viewerStatus->{'address'},
                'viewerproxyurl' => $viewerStatus->{'urluuid'} });
	    Log::Log4perl->get_logger('viewer')->trace("$viewer_uuid $project_name $viewer_name launched successfully with file(s): ", @file_path);
        ...
        $log->warn("doViewerVM: aborting viewer for [$project_name][$viewer_name] ret=$retCode");
    }
    if ($removeZip) {
    ...
    $log->info("LAUNCHVIEWER TIME: doViewerVM returns: $retCode seconds: ", time() - $start_time, "\n\n");
    if ( $retCode == OK ) {
        $retCode = 0;
    }
    elsif ( $retCode == NOTOK ) {
...
Name of the view to launch: Native, CodeDX, ThreadFix
...
The absolute path to the file containing the input assessment report. For now, this will be a single file, but for CodeDX, and ThreadFix, multiple input files can be specified.
