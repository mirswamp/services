...
# condor_chirp requres WorkingSpace
condor_chirp($bog{'intent'}, "TIME", "assessmentTask_exit", time());
cleanupWorkingSpace();
...
sub launchVM {
	...
	condor_chirp($bogref->{'intent'}, "TIME", "launchVM_start_vm", time());
    my ( $output, $status ) =
      systemcall(
"PERL5LIB=$basedir/perl5 $basedir/bin/start_vm --outsize 3072 --name $vmname $folder $vmplatform $imagefolder $vmfolder $nCPU $memMB"
    ...
    if ($status) {
		...
		condor_chirp($bogref->{'intent'}, "TIME", "launchVM_setDeadman", time());
        setDeadman(0);
        $nRestarts = 0;    # Clean slate.
        $ret = 1;
    }
	condor_chirp($bog{'intent'}, "TIME", "launchVM_return", time());
    return $ret;
}
...
sub processOutput {
    ...
    if ( ${$retryref} == 0 || $willRetry == 0 ) {
		...
		if ($preserveOutput == $PRESERVE_OUTPUT_ALWAYS || ($preserveOutput == $PRESERVE_OUTPUT_SUCCESS && $processResult) || ($preserveOutput == $PRESERVE_OUTPUT_FAILURE && ! $processResult)) {
			...
			# preserve original bog file
			cp($bogfile, $outfolder);
			# preserve bogref properties
			saveProperties("$outfolder/swamp-properties.bog", $bogref);
...
sub runAssessment {
	...
	condor_chirp($bog{'intent'}, "TIME", "runAssessment_openLicense", time());
    my $config = getSwampConfig();
	my $vmip = SWAMP::ToolLicense::getVMIPAddr($config, $vmname);
	(my $license_result, $vmip) = SWAMP::ToolLicense::openLicense($config, $bogref, $vmname, $vmip);
    updateExecutionResults($execrunid, {'vmip' => "$vmip"});
    ...
    while ( !$done ) {
        if ( doWaitLoop($execrunid) ) {
            ...
            #
			condor_chirp($bog{'intent'}, "TIME", "runAssessment_closeLicense", time());
			SWAMP::ToolLicense::closeLicense($config, $bogref, $license_result);
	...
	condor_chirp($bog{'intent'}, "TIME", "runAssessment_return", time());
    return;
}
...
sub sendExecutionResults {
    ...
    if (($state eq 'shutdown' || $state eq 'stopped') || (abs(time - $lastCall) > main->RETRY_RATE)) {
        $lastCall = time;
